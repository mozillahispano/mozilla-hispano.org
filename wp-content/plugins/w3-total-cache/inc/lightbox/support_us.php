...
<form action="admin.php?page=<?php echo $this->_page; ?>&amp;w3tc_save_support_us" method="post">
    <p>
    	<label>
        	...
        	<select name="support" class="select-support-type">
        		<option value=""><?php _e('select location', 'w3-total-cache'); ?></option>
        ...
        </label>, <input type="button" class="button button-tweet" value="<?php sprintf(_e('tell your friends%s with a tweet', 'w3-total-cache'), '" />'); ?>
        ...
        <?php sprintf(_e('and login to wordpress.org to give us a great %srating%s', 'w3-total-cache'),'<input type="button" class="button button-rating" value="', '" />.') ?>
    </p>
    ...
    <div style="text-align: center;">
        ...
        <p>
        	...
        	<?php echo $this->button_hide_note(__('Don\'t show this prompt again', 'w3-total-cache'), 'support_us'); ?>
        </p>
    </div>
</form>