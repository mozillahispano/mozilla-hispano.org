...
window.OnReady (
	function () {
		...
		var getStatusNavMainMenu = function(el) {
			/* ¿Usar?*/
			/* Ver si tiene la clase visibleChilds, o si el mainMenu no tiene display=block */
			if(hasClassName (el, "visibleChilds")) {
				return "visibleChilds";
			}
			else {
				if(window.getComputedStyle(mainMenu, null).getPropertyValue("display") === "block") {
					addClassName(mainMenu, "visibleChilds");
					return "visibleChilds";
				}
				else {
					return "";
